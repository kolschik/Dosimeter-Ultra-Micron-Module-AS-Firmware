...
void dac_reload()
{
  ...
  if((ADCData.DAC_voltage_raw > (DAC_GetDataOutputValue(DAC_Channel_2) + 5))    // вилка по напряжению +-1 бит
     || (ADCData.DAC_voltage_raw < (DAC_GetDataOutputValue(DAC_Channel_2) - 5)))
  {
    dac_off();
    dac_on();
